...
class App extends Component {
		...
		.then(res => {
			...
			.then(flights => {
				const filteredList = flights.filter((flight)=>{
					...
					else if(!isRoundTrip && flight.operationalDays.includes(departureDay)){
						return true;
					}
					else if(isRoundTrip && flight.operationalDays.includes(departureDay) && flight.operationalDays.includes(returnDay)){
						return true;
					}
				});

				filteredList.forEach(flight => {
					flight.price=flight.price*filters.passengerCount * (isRoundTrip?2:1);
	...
	renderFlightList = () => {
		if(this.state.error){
			return <div className="card no-data"><MessageBox text="Sorry, We have encountered a error" subText={ this.state.errorMessage} icon={Sorry} /> </div>
		}
		if(this.state.loading===undefined){
			return <div className="card no-data"><MessageBox text="Welcome to Flight Search" subText="Tell us where you want to go" icon={Airplane} /></div>
		}
		if(this.state.loading){
			return <Loader/>
		}
		if(this.state.filteredList.length == 0){
			return <div className="card no-data"><MessageBox text="Looks like there are no flights available" subText="Try changing the date or location" icon={Sorry} /> </div>
		}
		else{
			return <Fragment>
			<SearchParameters data={this.state.filters}/>
				<FlightList  filteredList={this.state.filteredList} isRoundTrip = {this.state.isRoundTrip}/>
			</Fragment>
		}
	}
