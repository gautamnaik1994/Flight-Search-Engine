...
class SearchBox extends Component {
	...
	render() {
		return (
			...
			<img src={Airplane} alt="Logo"/><span>Flight Search</span>
			</div>
				...
				<div className="row form-group">
					<div className="col-6">
						...
						<input
							...
							placeholder="eg. pnq"
						/>
						{this.state.errors['origin']!="" && this.state.origin=="" ? (<div className="error"> {this.state.errors['origin']} </div>):''}
					</div>
					<div className="col-6">
						...
						<input
							...
							placeholder="eg. del"
						/>
						{this.state.errors['destination']!="" && this.state.destination=="" ? (<div className="error"> {this.state.errors['destination']} </div>):''}
					</div>
				</div>
				<div className="row">
					<div className={`col-sm-12 col-md-${this.state.flightType === ROUNDTRIP ? '6':'12'}  form-group`}>
						...
						/>
						{this.state.errors['departureDate']!="" && this.state.departureDate=="" ? (<div className="error"> {this.state.errors['departureDate']} </div>):''}
					</div>
					{this.state.flightType === ROUNDTRIP && (
						<div className="col-sm-12 col-md-6  form-group">
							...
							/>
							{this.state.errors['returnDate']!="" && this.state.returnDate==""  ? (<div className="error"> {this.state.errors['returnDate']} </div>):''}
						</div>
					)}
				</div>
				...
				onChange={this.handleInputChange}
			/>
			{this.state.errors['passengerCount']!="" && (this.state.passengerCount=="" || this.state.passengerCount < 1) ? (<div className="error"> {this.state.errors['passengerCount']} </div>):''}
				</div>
				<button
